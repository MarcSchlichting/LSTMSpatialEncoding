...
#import environment
#import drone_sim.envs.environment as en
'''Global Variables'''
use_maps = False
...
class DronesEnv(gym.Env):
    ...
    def __init__(self):
        ...
        #self.environ = create_environment('env')
        
        print("All initialization procedures completed!")

    def step(self, action):
        no_active_vehicles = len(self.vm.active_vehicles)
        for idx_99 in range(no_active_vehicles):
            if action[idx_99][1]>0.15:
                action[idx_99][1] = 0.15
            elif action[idx_99][1]<-0.15:
                action[idx_99][1] = -0.15
    ...
    def reset(self,amount=20,xrange_init=[-100,100],yrange_init=[-100,100],xrange_target=[-100,100],yrange_target=[-100,100],min_distance=10,delta_t=1,eps_arr_abort=1.5,eps_cav=3,delta_cav=6,eps_arr=1.1,delta_arr=15,phi_segments=np.linspace(-np.pi,np.pi,num=25),r_segments=np.array([0,3,4,5,7,10,15,20]),max_stepcount=1000,arr_rew_amount=1000):
        ...
        self.amount = amount
        if self.amount == -1:
            file_name ="positions_destinations.txt"
            raw_pos = np.loadtxt(file_name)
            self.amount = raw_pos.shape[0]
    ...
    def create_vehicles(self,amount,min_dist, xorgrange,yorgrange,xdestrange,ydestrange):
        ...
        for idx in range(amount):
            ...
            if use_file==True:
                file_name ="positions_destinations.txt"
                raw_pos = np.loadtxt(file_name)
                trajectory = raw_pos[idx,0:2]
                destination = raw_pos[idx,2:]
            else:
